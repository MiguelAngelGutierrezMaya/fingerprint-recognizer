...
def calculate_frobenius_norm(channel, u, s_diagonalized, vt):
    ...
    for k in range(1, rank_channel+1):
        ...
        vt_k = vt[:k, :]
        channel_reconstruction_matrix = np.dot(
            np.dot(u_k, s_diagonalized_k), vt_k)
...
def calculate_optimal_k(singular_values, rank):
    ...
    for idx in range(0, end_idx):
        sig_diff = max(singular_values[idx:idx + 10]) - \
            min(singular_values[idx:idx + 10])
        if sig_diff < 0.32:
            optimal_k = idx
            break
...
def channel_via_optimal_k(k, u, s_diagonalized, vt):
    ...
    channel_vt_k = vt[:k, :]
    channel_reconstruction_matrix = np.dot(
        np.dot(channel_u_k, channel_s_diagonal_k), channel_vt_k)
...
def plot_frobenius_norm(red_frobenius_norm, blue_frobenius_norm, green_frobenius_norm, rank_channel):
    ...
    plt.plot(x, red_frobenius_norm, 'r', x, blue_frobenius_norm,
             'b', x, green_frobenius_norm, 'g')
    plt.show()
...
def svd_image(name):
    ...
    optimal_k = int(np.median(np.asarray(
        [optimal_k_red, optimal_k_blue, optimal_k_green])))
    ...
    red_frobenius_norm = calculate_frobenius_norm(
        red_channel, u_red, s_red_diagonalize, vt_red)
    blue_frobenius_norm = calculate_frobenius_norm(
        blue_channel, u_blue, s_blue_diagonalize, vt_blue)
    green_frobenius_norm = calculate_frobenius_norm(
        green_channel, u_green, s_green_diagonalize, vt_green)

    plot_frobenius_norm(red_frobenius_norm, blue_frobenius_norm,
                        green_frobenius_norm, rank_channel)

    blue_reconstruction_matrix = channel_via_optimal_k(
        optimal_k, u_blue, s_blue_diagonalize, vt_blue)
    green_reconstruction_matrix = channel_via_optimal_k(
        optimal_k, u_green, s_green_diagonalize, vt_green)
    red_reconstruction_matrix = channel_via_optimal_k(
        optimal_k, u_red, s_red_diagonalize, vt_red)

    re_image = cv2.merge((blue_reconstruction_matrix,
                          green_reconstruction_matrix, red_reconstruction_matrix))

    cv2.imwrite(_CDN_PATH + 'restored_' + name, re_image)
